...
exports.getAllUsers = (req, res, next) => {
  try {
    mycon.execute(
      'SELECT `user`.idUser,userkey.`key`,uservalue.`value` FROM `user` INNER JOIN uservalue ON uservalue.userId=`user`.idUser INNER JOIN userkey ON uservalue.keyId=userkey.idUserKey ORDER BY `user`.idUser ASC,userkey.keyOder ASC',
      (error, rows, fildData) => {
        if (!error) {
          res.send(rows);
        }
...
exports.getUserType = (req, res, next) => {
  try {
    mycon.execute(
      'SELECT `user`.utypeId FROM `user` WHERE `user`.idUser=' + req.body.uid,
      (error, rows, fildData) => {
        if (!error) {
          res.send(rows);
        }
...
exports.userLogin = (req, res, next) => {
  try {
    var q =
      "SELECT `user`.idUser,`user`.email,`user`.pword,`user`.mobileno,`user`.authcode,`user`.`status`,`user`.dateTime,`user`.utypeId FROM `user` WHERE `user`.idUser='" +
      req.body.email +
      "'";
      ...
      if (!e) {
        ...
        bcript.compare(req.body.pword, user.pword, (err, result) => {
          if (err) {
            ...
            return res
              .status(401)
              .json({ message: 'user name or password is wrong' });
          } else {
            if (result) {
...
exports.signUp = (req, res, next) => {
  try {
    ...
    var q =
      "SELECT `user`.idUser FROM `user` WHERE `user`.email='" +
      req.body.email +
      "'";
    mycon.execute(q, (e, r, f) => {
      if (!e) {
        if (r.length > 0) {
          ...
          bcript.hash(req.body.pword, 10, (err, hash) => {
            if (err) {
              ...
              console.log(hash);
              var qq =
                'INSERT INTO  `user`(  `email`, `pword`, `mobileno`, `authcode`, `status`, `dateTime`, `utypeId`)' +
                " VALUES ( '" +
                req.body.email +
                "', '" +
                hash +
...
exports.getUserKeys = (req, res, next) => {
  try {
    mycon.execute(
      'SELECT userkey.idUserKey,userkey.`key`,userkey.keyStatus,userkey.keyOder,userkey.formId,userkey.val,userkey.type FROM userkey WHERE userkey.keyStatus=1 ORDER BY userkey.keyOder ASC',
      (error, rows, fildData) => {
        if (!error) {
          res.send(rows);
        }
    ...
    mycon.execute(
      ...
      (e, r, f) => {
        if (!e) {
          ...
          mycon.execute(
            "UPDATE `user` SET `authcode`='" + val + "' WHERE `idUser`=" + uid,
            (er, ro, fi) => {
              if (!er) {
                console.log(ro);
              }
          ...
          textMg +=
            'Welcome to Smart Win Entrepreneur !' +
            '  Dear ' +
            name +
            ' your  SW No: ' +
            uid +
            ' and your verification number : ' +
            val +
            ' Please visit www.smartwinent.com and join smart win Telegram group https://bit.ly/3muQJRO  It is imperative to get rid of the product or service you have ordered from smart win within 30 days. (ඔබ විසින් ඇනවුම් කරනලද නිෂ්පාදනය හෝ සේවාව දින 30 ක් ඇතුලත smart win ආයතනයෙන් නිදහස් කරගැනිමට අනිවර්‍ය්යෙන්ම කටයුතු කලයුතු වේ) T and C Apply. Far inquiry call Tel. 037 22 34 777 WhatsApp / Telegram 0755443399' +
            '';
...
exports.getUsersList = (req, res, next) => {
  try {
    mycon.execute(
      'SELECT `user`.idUser,uservalue.`value`,DATE(`user`.dateTime) AS DAY FROM `user` INNER JOIN uservalue ON uservalue.userId=`user`.idUser INNER JOIN userkey ON uservalue.keyId=userkey.idUserKey WHERE uservalue.keyId=2 ORDER BY `user`.idUser ASC',
      (er, ro, fi) => {
        if (!er) {
          res.send(ro);
        }
...
exports.updateUserValues = (parm) => {
  let q =
    "UPDATE `uservalue` SET `value`='" +
    this.realEscapeString(parm.value) +
    "' WHERE `idUserValue`= " +
    parm.idUserValue;
...
exports.getUserData = (parm) => {
  let q =
    "SELECT uservalue.keyId,uservalue.`value`,userkey.`key` FROM uservalue INNER JOIN userkey ON uservalue.keyId=userkey.idUserKey WHERE uservalue.userId='" +
    parm.id +
    "' ORDER BY userkey.keyOder ASC";
  mycon.execute(q, (er, ro, fi) => {
    if (!er) {
...
exports.getUserDataByPin = (req, res, next) => {
  let q =
    'SELECT sw_tree.userId,userkey.idUserKey,userkey.`key`,userkey.keyStatus,userkey.keyOder,userkey.formId,userkey.type,uservalue.`value` FROM sw_tree INNER JOIN uservalue ON uservalue.userId=sw_tree.userId INNER JOIN userkey ON uservalue.keyId=userkey.idUserKey WHERE sw_tree.swTreeId=' +
    req.body.tid;
  mycon.execute(q, (er, ro, fi) => {
    if (!er) {
      res.send(ro);
...
exports.sendBulkSms = (req, res, next) => {
  try {
    mycon.execute(
      'SELECT uservalue.userId,uservalue.`value`,userkey.`key` FROM uservalue INNER JOIN userkey ON uservalue.keyId=userkey.idUserKey WHERE uservalue.keyId=9 AND uservalue.`value` IS NOT NULL',
      (error, rows, fildData) => {
        if (!error) {
          let arr = [];
        ...
        if (!er) {
          ...
          mycon.execute(
            ...
            (e, r, n) => {
              if (!e) {
                var textMg =
                  'Your System Ref Code is : SR ' +
                  ref +
                  ' Product Cord SWE-' +
                  req.body.product +
                  ' PV- 01 Pay LKR. ' +
                  req.body.price +
                  ' Sampath Bank Kurunegala Super Branch Smart Win Enterpreneur (Privet) Limited current Acc. No. 000610016871. PEOPLES BANK Kurunegala maliyadewa  main Branch Acc: 226 100 1800 63581 ඉහත සදහන් බැංකු ගිණුම් වලින් ඔබට පහසු ගිණුමකට මුදල් තැන්පත්කල හැකි අතර මුදල් තැන්පත් කිරිමේදි තැන්පතු පතේ මුදල් තැන්පත් කිරිමට හේතුව  තිරුවේ SR අංකය සදහන් කරන්න  සම්පත් බැංකුවේ ගිණුමට මුදල් තැන්පත් කිරිමේදි පමණක් CDM යන්ත්‍රයෙන් මුදල් තැන්පත්කල හැකි අතර එවිට Reference යටතේ  SR අංකය සටහන් කරන්න. සම්පත් බැංකුවේ ගිණුමට මුදල් තැන්පත් කිරිමේදි පමණක් CDM යන්ත්‍රයෙන් මුදල් තැන්පත්කල හැකි අතර එවිට Reference යටතේ  SR අංකය සටහන් කරන්න. It is imperative to get rid of the product or service you have ordered from smart win within 30 days. (ඔබ විසින් ඇනවුම් කරනලද නිෂ්පාදනය හෝ සේවාව දින 30 ක් ඇතුලත smart win ආයතනයෙන් නිදහස් කරගැනිමට අනිවර්‍ය්යෙන්ම කටයුතු කලයුතු වේ) T and C Apply. Far inquiry call 0372234777 WhatsApp / Telegram 0755443399';
...
exports.getCoreLeaders = (req, res, next) => {
  try {
    mycon.execute(
      'SELECT leader_main.id,leader_main.main,leader_main.`name` FROM leader_main',
      (er, ro, fi) => {
        if (!er) {
          res.send(ro);
        } else {
...
exports.rejectBankPros = (req, res, next) => {
  try {
    mycon.execute(
      'UPDATE `refaral` SET `status`=3 WHERE `id`=' + req.body.ref,
      (er, ro, fi) => {
        if (!er) {
          res.send(ro);
        } else {
...
exports.completeBankPros = (req, res, next) => {
  try {
    mycon.execute(
      'UPDATE `refaral` SET `status`=2 WHERE `id`=' + req.body.ref,
      (er, ro, fi) => {
        if (!er) {
          res.send(ro);
        } else {
