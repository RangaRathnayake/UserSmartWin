        ...
        mycon.execute("INSERT INTO `refaral` (`ref`,`iSWno`,`namewith`,`nic`,`address`,`mobile`,`payType`,`status`,`userId`,`day`,`product`,`price`) VALUES (''," + req.body.iSWno + ",'" + this.realEscapeString(req.body.name) + "','" + req.body.nic + "','" + this.realEscapeString(req.body.address) + "','" + req.body.mobile + "','" + req.body.type + "',0," + req.body.userId + ",'" + day + "','" + req.body.product + "','" + req.body.price + "')", (er, ro, fi) => {
            if (!er) {
                ...
                mycon.execute("UPDATE `refaral` SET `ref`='" + ref + "' WHERE `id`=" + ro.insertId, (e, r, n) => {
                    if (!e) {
                        var textMg = "Your System Ref Code is : SR " + ref + " Product Cord SWE-" + req.body.product + " PV- 01 Pay LKR. " + req.body.price + " Sampath Bank Kurunegala Super Branch Smart Win Enterpreneur (Privet) Limited current Acc. No. 000610016871. PEOPLES BANK Kurunegala maliyadewa  main Branch Acc: 226 100 1800 63581 ඔබ විසින් ඇනවුම් කරනලද නිෂ්පාදනය හෝ සේවාව දින 30 ක් ඇතුලත smart win ආයතනයෙන් නිදහස් කරගැනිමට අනිවර්‍ය්යෙන්ම කටයුතු කලයුතු වේ. (It is imperative to get rid of the product or service you have ordered from smart win within 30 days.) T and C Apply. Far inquiry call 0372234777 WhatsApp / Telegram 0755443399 ";
